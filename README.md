...
- **Frontend**: 基于 Vue.js 的前端应用
- **Session**: 用户与 Claude AI 的对话会话
- **Message**: 会话中的单条消息（用户消息或助手消息）
- **Tool_Result**: Claude 执行工具后返回的结果
- **VNC_Connection**: 到虚拟机的 VNC 远程桌面连接
- **Database**: SQLite 数据库用于持久化存储
- **SSE**: Server-Sent Events，服务器推送事件
- **Sampling_Loop**: Claude API 的采样循环，处理对话和工具调用
